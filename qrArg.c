...
mat matrix_new(int m, int n)
{
	...
	x->v[0] = calloc(m * n, sizeof(double));
	for (int i = 0; i < m; i++)
		x->v[i] = x->v[0] + n * i;
	x->m = m;
	x->n = n;
...
void householder(int n, mat m, mat* R, mat* Q) //assuming nxn matrix
{
	//*/for VS
	mat* q = malloc(sizeof(mat_t)*n);
	...
	//mat q[m->m]; //doesn't work in VS
	mat z = m, z1;
	for (int k = 0; k < m->n && k < m->m - 1; k++) {
		//double e[m->m], x[m->m], a; //doesn't work in VS
		z1 = matrix_minor(z, k);
	...
	*R = matrix_mul(q[0], m);
	...
	//matrix_delete(q[0]); //q[0] is freed in the original code, but it should not be.
	z = matrix_mul(*Q, m);
	matrix_delete(*R);
	*R = z;
...
int main()
{
	...
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			A[i][j] = As[i][j];
	mat R = NULL, Q = NULL;
	mat T1 = matrix_copy2(n, A, n);
	mat T2 = NULL;
	mat U = matrix_newI(n, n);
	mat At = NULL;
	for (i = 0; i < MaxIter; i++) {
		//T1 = QR
		householder(n, T1, &R, &Q);
